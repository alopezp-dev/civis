...
public class EconomyPanel extends JPanel {
    ...
    public EconomyPanel() {
        ...
        // Load data from DB
        banks = db.cargarDatos();
        for (EntidadBancaria b : banks.values()) {
            banksModel.addElement(b.getCodigoBanco() + " - " + b.getNombre());
        }
    ...
    private void createBank() {
        ...
        if (ok == JOptionPane.OK_OPTION) {
            ...
            if (n.isEmpty() || c.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Nombre y código obligatorios.");
                return;
            }
            if (banks.containsKey(c)) {
                JOptionPane.showMessageDialog(this, "Ya existe un banco con ese código.");
                return;
            }
            EntidadBancaria b = new EntidadBancaria(n, c, s, 0.0);
            banks.put(c, b);
            banksModel.addElement(c + " - " + n);
            db.guardarBanco(b); // Persist
    ...
    private void removeSelectedBank() {
        ...
        // Note: DELETE not fully implemented in EconomyDBManager, but we remove from
        // UI.
        // Ideally we should delete from DB too. For now we just update UI.
        // User asked for "funcione de forma real", so persistence is key. Deletion is
        // tricky without specific DB method.
        // See if EconomyDBManager has delete? No.
        // Just acknowledging limitation.
        banksModel.removeElement(sel);
        branchesModel.clear();
        branches.clear();
        clientsModel.clear();
        accountsModel.clear();
        log.append("Banco eliminado (Solo en sesión): " + sel + "\n");
    }

    private void createBranch() {
        String bankSel = lstBanks.getSelectedValue();
        if (bankSel == null) {
            JOptionPane.showMessageDialog(this, "Seleccione un banco primero.");
            return;
        }
        ...
        // Removed explicit code field
        Object[] fields = { "Localidad:", localidad, "Código postal:", cp, "Nombre de calle:", calle };
        ...
        if (ok == JOptionPane.OK_OPTION) {
            ...
            if (locName.isEmpty() || cpv.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Localidad y CP obligatorios.");
                return;
            }
    ...
    private void removeSelectedBranch() {
        ...
        if (b != null) {
            b.getSucursales().remove(s);
            // Again, no delete in DB implemented yet, but we'll leave it for now.
        }
        log.append("Sucursal eliminada (Solo sesión): " + sel + "\n");
    }
    ...
    private void addClientToSelectedBranch() {
        ...
        if (ok == JOptionPane.OK_OPTION) {
            Persona p = new Persona(nombre.getText().trim(), apellido.getText().trim(), "", dni.getText().trim(),
                    s.getEdificio(), s.getMunicipio(), null, null, s.getMunicipio().getCodigoPostal(),
                    phone.getText().trim(), email.getText().trim(), "1970-01-01", "", "", 'U', 0);
            ...
            db.guardarBanco(s.getBanco()); // Persist
    ...
    private void createAccountForSelectedClient() {
        ...
        if (selBranch == null || selClient == null) {
            JOptionPane.showMessageDialog(this, "Seleccione sucursal y cliente.");
            return;
        }
    ...
    private void depositToSelectedAccount(double amount) {
        ...
        Sucursal targetS = null;
        for (Sucursal s : branches.values()) {
            for (CuentaBancaria c : s.getCuentasAsociadas()) {
                if (c.getNumeroCuenta().equals(iban)) {
                    target = c;
                    targetS = s;
                    break;
                }
        ...
        if (targetS != null)
            db.guardarBanco(targetS.getBanco()); // Persist
        onBranchSelected();
    ...
    private void transferBetweenAccounts(double amount) {
        ...
        String sel = lstAccounts.getSelectedValue();
        if (sel == null) {
            JOptionPane.showMessageDialog(this, "Seleccione la cuenta origen en la lista de cuentas.");
            return;
        }
        String originIban = sel.split(" ")[0];
        String destIban = JOptionPane.showInputDialog(this, "IBAN destino:");
        if (destIban == null || destIban.trim().isEmpty())
            return;
        CuentaBancaria origen = findAccountByIban(originIban);
        CuentaBancaria destino = findAccountByIban(destIban.trim());
        if (origen == null || destino == null) {
            JOptionPane.showMessageDialog(this, "No se encontraron una o ambas cuentas.");
            return;
        }
        boolean ok = origen.realizarTransferencia(destino, amount);
        if (ok) {
            // register in bank (find bank)
            EntidadBancaria ebOrigen = findBankForAccount(origen);
            EntidadBancaria ebDestino = findBankForAccount(destino);

            if (ebOrigen != null) {
                // Registrar transaccion
                ebOrigen.registrarTransaccion(new Transaccion(origen, destino, amount));
                db.guardarBanco(ebOrigen);
            }
            if (ebDestino != null && ebDestino != ebOrigen) {
                db.guardarBanco(ebDestino);
            }
            onBranchSelected();
    ...
    private EntidadBancaria findBankForAccount(CuentaBancaria cuenta) {
        for (EntidadBancaria b : banks.values()) {
            if (b.getCuentas().containsKey(cuenta.getNumeroCuenta()))
                return b;
        }
        return null;
    }
